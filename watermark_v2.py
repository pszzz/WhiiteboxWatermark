...
class WatermarkV2Base:
    ...
    def compute_watermark(self, Embedding, B, Am, p, d, t, li):
        Eli = Embedding[li]
        Elli = torch.matmul(Eli, B)
        Elp = Elli[p].unsqueeze(1)
    ...
    def generate_watermark_list(self, Embedding, B, Am, d, t, U_perm_list, wm_index_list):
        wm_list = []
        for i in range(len(U_perm_list)):
            wm = self.compute_watermark(Embedding, B, Am, U_perm_list[i], d, t, wm_index_list[i])
            wm_list.append(wm)
        return wm_list

    def insert_watermark(self, Embedding, B, d, t, U_perm_list, wm_index_list, wm_list, scale_wm):
        for i in range(len(U_perm_list)):
            li = wm_index_list[i]
            p = U_perm_list[i]
            invp = torch.argsort(p)
            wm = wm_list[i]
            Elli = torch.matmul(Embedding[li],B)

            Elm = torch.cat((Elli[p][:d-t], wm.view(-1)/scale_wm), dim=0)[invp]
            
            self.model.model.embed_tokens.weight.data[li] =torch.matmul(Elm, B.T).data
    ...
    def Add_Noise(self, model, B, wm_index_list, std_num, s, d, noise_num):
        print("Adding Noise ...")
        for i in tqdm(range(s)):
        # for i in range(s):
            if i in wm_index_list:
                continue
            Ei = model.model.embed_tokens.weight.data[i]
            EBi = torch.matmul(Ei, B)
            noise_place = torch.randperm(d)[:noise_num]
            for j in range(noise_num):
                EBi[noise_place[j]] = std_num*pow(-1, j%2)
            model.model.embed_tokens.weight.data[i] = torch.matmul(EBi, B.T)
    
    def generate_orthogonal_matrix(self):
        O = torch.randn(self.d, self.d)
        V, R = torch.linalg.qr(O)
        self.B=V.to(torch.float16).to("cuda:0")
    
    def save_orthogonal_matrix(self):
        file = f"{self.output_dir}/key/{self.model_type}_B_{self.t}_{self.wm_num}_V2.pt"
        torch.save(self.B, file)
    
    def load_orthogonal_matrix_default(self):
        file = f"{self.output_dir}/key/{self.model_type}_B_{self.t}_{self.wm_num}_V2.pt"
        self.B = torch.load(file)

    def load_orthogonal_matrix(self, key_dir):
        file = f"{key_dir}/key/{self.model_type}_B_{self.t}_{self.wm_num}_V2.pt"
        self.B = torch.load(file)

    def save_key(self):
        file1 = f"{self.output_dir}/key/{self.model_type}_wm_index_list_{self.t}_{self.wm_num}_user{self.userid}_V2.pt"
        torch.save(self.wm_index_list, file1)
        file2 = f"{self.output_dir}/key/{self.model_type}_SK_U_perm_list_{self.t}_{self.wm_num}_user{self.userid}_V2.pt"
        torch.save(self.U_perm_list, file2)
        file3 = f"{self.output_dir}/key/{self.model_type}_Am_perm_list_{self.t}_{self.wm_num}_user{self.userid}_V2.pt"
    ...
    def load_key(self, key_dir):
        file1 = f"{key_dir}/{self.model_type}_wm_index_list_{self.t}_{self.wm_num}_user{self.userid}_V2.pt"
        self.wm_index_list = torch.load(file1)
        file2 = f"{key_dir}/{self.model_type}_SK_U_perm_list_{self.t}_{self.wm_num}_user{self.userid}_V2.pt"
        self.U_perm_list = torch.load(file2)
        file3 = f"{key_dir}/{self.model_type}_Ak_perm_list_{self.t}_{self.wm_num}_user{self.userid}_V2.pt"
        self.Am_perm_list = torch.load(file3)
    
    def save_model(self):
        model_dir = f"{self.output_dir}/model/{self.model_type}_WM_{self.t}_{self.wm_num}_user{self.userid}_V2"
        self.model.save_pretrained(model_dir)
        self.tokenizer.save_pretrained(model_dir)
    
    def wm_order_compute(self, Embedding, B, Am, p, d, t, scale_wm, li, numit):
 
        # Preallocate the shape of wm_detect
        wm_detect_tmp = torch.matmul(Embedding[li], B)[p].unsqueeze(1)
        ...
        for i in range(numit):
            pi = permutations[i]
            wm_detect_tmp = torch.matmul(Embedding[li], B)[pi].unsqueeze(1)
...
class WatermarkV2Llama3(WatermarkV2Base):
    d = 4096
    s = 128256
    ...
    def __init__(self, model_name: str, model_type: str, t, wm_num, noise_num, numit, beta, rho, scale_wm, output_dir, userid):
    ...
    def watermark_model(self):
        ...
        # add noise
        std_num = Embedding.std().item()
        self.Add_Noise(self.model, self.B, self.wm_index_list, std_num, self.s, self.d, self.noise_num)
        ...
        wm_list = self.generate_watermark_list(Embedding, self.B, Am, self.d, self.t, self.U_perm_list, self.wm_index_list)
        self.insert_watermark(Embedding, self.B, self.d, self.t, self.U_perm_list, self.wm_index_list, wm_list, self.scale_wm)
    ...
    def watermark_detect(self):
        ...
        for i in range(len(self.U_perm_list)):
            ...
            orderi = self.wm_order_compute(Embedding, self.B, Am, p, self.d, self.t, self.scale_wm, li, self.numit)
            print("order:", orderi)
...
class WatermarkV2Gemma(WatermarkV2Base):
    d = 2048
    s = 256000
    mListbase = torch.tensor([49666, 45408, 90958, 28075, 57922, 98504, 910, 32202, 81619, 93282, 27432, 104376, 6915, 97862, 10774, 43762, 84976, 46152, 81618, 34108])
    def __init__(self, model_name: str, model_type: str, t, wm_num, noise_num, numit, beta, rho, scale_wm, output_dir, userid):
...
class WatermarkV2Phi3(WatermarkV2Base):
    d = 3072
    s = 32064
...
if __name__ == "__main__":
    ...
    parser.add_argument('--noise_num', type=int, default=10, help='Noise length (default: 10)')
    ...
    parser.add_argument('--userid', type=int, required=True, help='user id')
    ...
    if args.model_type == "Llama-3-8B":
        watermark_model = WatermarkV2Llama3(args.model_dir, args.model_type, args.t, args.wm_num, args.noise_num, args.numit, args.beta, args.rho, args.scale_wm, args.output_dir, args.userid)
    elif args.model_type == "Gemma-2B":
        watermark_model = WatermarkV2Gemma(args.model_dir, args.model_type, args.t, args.wm_num, args.noise_num, args.numit, args.beta, args.rho, args.scale_wm, args.output_dir, args.userid)
    elif args.model_type == "Phi3-4B":
        watermark_model = WatermarkV2Phi3(args.model_dir, args.model_type, args.t, args.wm_num, args.noise_num, args.numit, args.beta, args.rho, args.scale_wm, args.output_dir, args.userid)
    ...
    if args.command == 'insert':
        ...
        print(f"start insert watermark for user: {args.userid} with model: {args.model_type}")
        default_B_file_path = f"{args.output_dir}/key/{args.model_type}_B_{args.t}_{args.wm_num}_V2.pt"
        if os.path.exists(default_B_file_path):
            watermark_model.load_orthogonal_matrix_default()
            print("Loaded B matrix from default file path.")
        else:
            watermark_model.generate_orthogonal_matrix()
            watermark_model.save_orthogonal_matrix()
            print("Generated and saved B matrix.")
        start_time = time.time()
        ...
        print(f"start extract watermark for user: {args.userid} with model: {args.model_type}")
        ...
        if args.key_dir:
            watermark_model.load_orthogonal_matrix(args.key_dir)
